...
class Stripe {
	...
	public function setup_hooks() {
		...
		add_filter('sos/project/payment/stripe/paymentlink', [$this, 'thePaymentlink'], 10, 2);
		add_filter('sos/project/payment/stripe/handlesuccess', [$this, 'handleSuccess'], 10, 2);
		add_filter('sos/project/payment/stripe/payment_methods', [$this, 'paymentMethods'], 10, 0);
		add_filter('sos/project/payment/stripe/payment_history', [$this, 'paymentHistory'], 10, 2);
		add_filter('sos/project/payment/stripe/subscriptionToggle', [$this, 'subscriptionToggle'], 10, 3);
		add_filter('sos/project/payment/stripe/subscriptionCancel', [$this, 'subscriptionCancel'], 10, 3);
		add_filter('sos/project/payment/stripe/getsubscriptionby', [$this, 'getSubscriptionBy'], 10, 2);
		add_filter('sos/project/payment/stripe/switchpaymentcard', [$this, 'switchPaymentCard'], 10, 2);

		add_filter('sos/project/payment/stripe/allowswitchpause', [$this, 'allowSitchPause'], 10, 3);

		add_filter('sos/project/rewrite/rules', [$this, 'rewriteRules'], 10, 1);
	...
	public function init() {
		$this->stripePublishAble = apply_filters('sos/project/system/getoption', 'stripe-publishablekey', false);
		$this->stripeSecretKey = apply_filters('sos/project/system/getoption', 'stripe-secretkey', false);
	}
	...
	public function template_include($template) {
		...
		if ($payment_status && ! empty($payment_status) && ($file = SOSPOPSPROJECT_DIR_PATH . '/templates/stripe/payment_status.php') && file_exists($file) && ! is_dir($file)) {
			return $file;
		} else {
			return $template;
		}
	...
	public function create_stripe_checkout_session($args = false, $client_reference_id = false) {
		...
		$param = [
			...
			'line_items'								=> [
				...
				// 			'images' => [esc_url(SOSPOPSPROJECT_BUILD_URI . '/icons/Online payment_Flatline.svg')],
				// 		],
				// 	]
				//]
			],
	...
	public function createStripeCustomer($email, $source) {
		...
		return $jsonResponse['id']; // Return the customer ID
	}	
