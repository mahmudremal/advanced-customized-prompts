...
class Option {
  ...
  private $file;
	...
	protected function setup_hooks() {
		$this->file = SOSPOPSPROJECT__FILE__;
		$this->plugin_slug = 'sospopsprompts';
		$this->plugin_name = 'sospopsprompts';
		$this->textdomain = 'sospopsprompts'; // str_replace('_', '-', $plugin_slug);
		// Initialise settings
	...
	public function init() {
		$this->general = (object) wp_parse_args(apply_filters('sos/project/settings/general', []), [
			...
			'page_header'					=> __('Custom pops setup', 'sospopsprompts'),
			'page_subheader'				=> __('Customization popup for frontend processes including search, filter & gathering client idea.', 'sospopsprompts'),
			'no_password'					=> __('A password is required.', 'sospopsprompts'),
		]);
	}
	...
	public function add_settings_link($links) {
		$settings_link = '<a href="options-general.php?page='.$this->general->slug.'">' . __('Settings', 'sospopsprompts') . '</a>';
		// array_push($links, $settings_link);
		$links[] = $settings_link;
		// $links[] = '<a href="https://www.fiverr.com/mahmud_remal/" target="_blank">' . __('Developer Support', 'sospopsprompts') . '</a>';
		return $links;
	}
	/**
